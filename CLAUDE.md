...
### ビルドプロセス
ビルドコマンド (`bun run build`) は以下の処理を順次実行します：
1. `copy:assets`: assets/ディレクトリの画像をpublic/assets/にコピー
2. `generate:ogp`: 各記事のOGP画像を生成
3. `next build`: Next.jsの静的サイト生成
...
### PRテンプレートの内容
`.github/pull_request_template.md`が存在し、以下のセクションが含まれています：

- **概要**: 変更の目的と背景
- **変更内容**: 具体的な変更点のリスト
- **変更の種類**: feat/fix/docs/style/refactor/perf/test/build/ciのチェックボックス
- **テスト方法**: 動作確認の手順
- **チェックリスト**: 
  - コードレビューのセルフチェック
  - ローカルでのテスト実施
  ...
  - 破壊的変更の有無
  - 依存関係の更新確認
- **関連Issue**: 関連するIssue番号（#123形式）
- **スクリーンショット**: UI変更がある場合の画像添付（オプション）
...
### Claude Code使用時の注意
Claude Codeを使用してPRを作成する際は、**必ずタイトルと説明文を日本語で生成**してください。英語での自動生成は禁止です。
...
詳細は[コミットメッセージ規約](./docs/commit-convention.md)を参照してください。

## Commit/Push前の必須確認事項

### 重要：コードの変更をcommit/pushする前に必ず以下を実行

変更をcommitまたはpushする前に、**必ず**以下のコマンドを実行してエラーがないことを確認してください：
...
- [ ] `git diff`で差分を確認する
- [ ] `bun run build`が成功する
- [ ] `bun run lint`でエラーがない
- [ ] 変更した機能が正しく動作する
- [ ] 既存の機能に影響がない
- [ ] コミットメッセージが規約に従っている

### 注意事項
- これらの確認を行わずにcommit/pushすることは**禁止**
- CIでビルドが失敗した場合は、速やかに修正する
- 特に大きな変更を行った場合は、より詳細なテストを実施する
